...
function requirePeriodAmount(value: number): number {
  if (!Number.isSafeInteger(value) || value < 0) {
    throw new Error('日期周期必须是非负整数');
  }
  return value;
}
...
export function isDatePeriodUnit(value: unknown): value is DatePeriodUnit {
...
export function addDateOnlyPeriod(
  ...
  const safeAmount = requirePeriodAmount(amount);
  if (safeAmount === 0) return formatParts(source);

  if (unit === 'day') {
    const result = new Date(Date.UTC(source.year, source.month - 1, source.day + safeAmount));
    const parts = {
  ...
  if (unit === 'month') {
    const targetIndex = source.year * 12 + source.month - 1 + safeAmount;
  ...
  const year = source.year + safeAmount;
  assertSupportedYear(year);
  return formatParts({
    year,
    month: source.month,
...
export function advanceDateOnlyToAtLeast(
...
): { expiryDate: string; periodsAdvanced: number } {
  requireDateOnly(expiryDate);
  requireDateOnly(targetDate);
  if (!Number.isSafeInteger(periodValue) || periodValue <= 0) {
    throw new Error('续费周期必须是正整数');
  }
  if (compareDateOnly(expiryDate, targetDate) >= 0) {
    return { expiryDate, periodsAdvanced: 0 };
  }

  if (periodUnit === 'day') {
    const daysBehind = differenceInCalendarDays(expiryDate, targetDate);
    const periodsAdvanced = Math.ceil(daysBehind / periodValue);
    return {
      expiryDate: addDateOnlyPeriod(expiryDate, periodsAdvanced * periodValue, 'day'),
      periodsAdvanced,
    };
  }
  ...
  while (compareDateOnly(nextExpiryDate, targetDate) < 0) {
    nextExpiryDate = addDateOnlyPeriod(nextExpiryDate, periodValue, periodUnit);
    periodsAdvanced += 1;
    if (periodsAdvanced > 120000) throw new Error('续费日期计算超出安全范围');
  }
  return { expiryDate: nextExpiryDate, periodsAdvanced };
}
...
export function getDateOnlyInTimeZone(now: Date, timezone: string): string {
...
export function getCalendarHoursUntilDateEnd(
  ...
  const today = formatParts(localNow);
  const dayDifference = differenceInCalendarDays(today, expiryDate);
