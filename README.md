...
- **自动续费** — 按系统时区的日历日期续期，正确处理月底、闰年和跨多个遗漏周期的追赶，并记录手动/自动续费历史
- **续费中心** — 集中展示待续、过期与自动续费订阅，支持快速/批量续费、批量切换自动续费、六个月费用预测和全局续费历史
- **通知模板** — 自定义通知模板，实时预览，支持名称、类型、到期日、状态、剩余天数、费用、周期、自动续费、农历、备注、时间、时区等变量
- **通知日志** — 查看所有通知发送记录，支持按订阅/渠道/状态/日期筛选
- **导入/导出** — JSON 完整备份订阅与续费历史，CSV 支持多行字段和表格编辑；重复恢复会自动跳过相同订阅
- **费用统计** — 订阅费用按月/年汇总，按类型和分类分布，费用明细表，支持可搜索多币种选择，每日自动获取汇率换算
- **统一时区** — 前端仪表盘、日历、统计与后端提醒统一使用系统配置时区
- **农历支持** — 日期显示农历，周期可按农历计算
- **深色模式** — 侧边栏一键切换深色/浅色主题
- **响应式布局** — 移动端自适应，侧边栏抽屉模式，日历卡片适配
- **按需加载** — 页面、弹窗和 Element Plus 组件分包加载，并在 CI 中限制单个 JavaScript 包体积
...
| ![续费中心](screenshots/renewal-center.jpg)  | ![日历视图](screenshots/calendar.jpg)  |
